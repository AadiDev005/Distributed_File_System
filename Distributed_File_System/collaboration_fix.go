...
func (efs *EnterpriseFileServer) InitializeCollaboration() {
	...
	// Fix: Use efs.mux instead of http
...
// Enhanced document change handler with Operational Transform
func (efs *EnterpriseFileServer) handleDocumentChangeWithOT(client *CollabClient, payload interface{}) {
	data, _ := json.Marshal(payload)
	var changeData struct {
		DocumentID string    `json:"documentId"`
	...
	// Initialize operational transform if not exists
	if efs.operationalTransform == nil {
		efs.operationalTransform = &OperationTransform{}
	}

	// Transform operation against concurrent operations
	var transformedOp = changeData.Operation

	// Check for concurrent operations from other users
	if len(doc.Changes) > 0 {
		lastChange := doc.Changes[len(doc.Changes)-1]
		lastOp := Operation{
			Type:      lastChange.Type,
			Position:  lastChange.Position,
			Content:   lastChange.Content,
			Author:    lastChange.UserID,
		...
		_, transformed2 := efs.operationalTransform.Transform(lastOp, changeData.Operation)
		transformedOp = transformed2

		log.Printf("🔄 Operational Transform applied for user %s in doc %s", client.Name, changeData.DocumentID)
	}

	// Apply transformed operation to document content
	newContent, err := efs.operationalTransform.ApplyOperation(doc.Content, transformedOp)
	if err != nil {
		log.Printf("❌ Failed to apply operation: %v", err)
		client.sendMessage("operation-error", map[string]interface{}{
			"error":      err.Error(),
	...
	// Create document change for audit trail
	change := DocumentChange{
		ID:         fmt.Sprintf("change-%d-%s", time.Now().UnixNano(), client.ID),
		DocumentID: changeData.DocumentID,
		UserID:     client.ID,
	...
	// Broadcast transformed operation to all other collaborators
	efs.broadcastToDocumentCollaborators(doc.ID, client.ID, "document-change", map[string]interface{}{
		"documentId": changeData.DocumentID,
	...
	client.sendMessage("operation-applied", map[string]interface{}{
		"documentId": changeData.DocumentID,
		"version":    doc.Version,
		"success":    true,
	})

	log.Printf("📝 Document %s updated by %s (v%d)", changeData.DocumentID, client.Name, doc.Version)
}

// Broadcast to all collaborators except sender
func (efs *EnterpriseFileServer) broadcastToDocumentCollaborators(documentID, senderID, messageType string, payload interface{}) {
	efs.collaborationMutex.RLock()
	doc := efs.collaborationDocs[documentID]
	efs.collaborationMutex.RUnlock()
	...
	doc.mutex.RLock()
	collaborators := make([]*CollabClient, 0, len(doc.Collaborators))
	for _, collaborator := range doc.Collaborators {
		if collaborator.ID != senderID && collaborator.IsOnline {
	...
	for _, collaborator := range collaborators {
		select {
		case collaborator.send <- messageBytes:
		default:
			log.Printf("⚠️ Failed to send to collaborator %s", collaborator.Name)
		}
	}
}
